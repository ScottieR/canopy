...
use canopy_lib::models::{Agent, AgentStatus, AgentPersonality, AgentCapabilities, AgentStats};
...
async fn test_workspace_file_upload_and_read_base64() {
    ...
    assert!(upload_result.is_ok(), "Failed to upload workspace file: {:?}", upload_result);
    
    // Test 2: Read workspace file back as base64 and verify MIME type and content matches
    let read_result = read_workspace_file_base64(
        db_state.clone(),
        agent_id.clone(),
        filename.clone(),
    ).await;
    assert!(read_result.is_ok(), "Failed to read workspace file: {:?}", read_result);
    
    let returned_data_url = read_result.unwrap();
    assert!(returned_data_url.starts_with("data:image/png;base64,"), "MIME type mismatch: {}", returned_data_url);
    assert!(returned_data_url.contains("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJ"), "Data content mismatch");
    ...
    assert!(upload_traversal.is_err(), "Path traversal in upload was not rejected");
    assert_eq!(upload_traversal.unwrap_err(), "Invalid filename");

    // Test 4: Path traversal rejection in read
    let read_traversal = read_workspace_file_base64(
        db_state.clone(),
        agent_id.clone(),
        invalid_filename,
    ).await;
    assert!(read_traversal.is_err(), "Path traversal in read was not rejected");
