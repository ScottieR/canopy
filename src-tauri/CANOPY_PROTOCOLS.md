...
- **Requesting an API key for any provider**: Use the generic `api_key` companion when you need a plain API token/key for a service Canopy doesn't natively ship. Output: `[request_connection: api_key?providerName=<Display Name>&tokenUrl=<https URL where the user finds the key>&instructions=<short plain-text steps>&secretKey=<OPTIONAL_ENV_STYLE_NAME>&placeholder=<optional input hint>]` (URL-encode all values). Example: `[request_connection: api_key?providerName=Seats.aero&tokenUrl=https%3A%2F%2Fseats.aero%2Faccount&instructions=Sign%20in%20to%20your%20Seats.aero%20Pro%20account%2C%20open%20the%20Developer%20tab%2C%20and%20copy%20your%20API%20Key.&placeholder=sk_...]`. Rules: `providerName` is required; `tokenUrl` must be an absolute http(s) URL (anything else is dropped); `instructions` is rendered as plain text and capped at 600 characters; if `secretKey` is omitted the name is derived from the provider (e.g. `SEATS_AERO_API_KEY`). In the main app, the user pastes the key into the companion window. **Over Slack, this same tag is automatically rewritten into a plain `https://` link** the user can open on their phone and paste the key into directly — you don't need to do anything differently, and you don't need to explain the mechanism; just write your message normally and the link (canopy:// in-app, or https:// over Slack) is appended for you. Either way it is saved to the Keychain as `agent_<yourAgentId>_<SECRET_NAME>`. The raw value is NEVER shown to you — request it at runtime through the JIT credential flow using that credential id.
- **Proactively minting a web connection link** (advanced, rarely needed): if you specifically need the `https://` capture link itself — e.g. to paste into a channel that isn't Slack, or to reference in your own custom message instead of relying on the automatic `[request_connection: api_key?...]` rewrite — POST to the JIT bridge instead of emitting the tag. See `PERMISSIONS.md` in your workspace ("Requesting a provider API key link") for the exact request shape, auth header, and response format. Prefer the plain `[request_connection: api_key?...]` tag for normal use — this is only for when you need the raw URL yourself.
