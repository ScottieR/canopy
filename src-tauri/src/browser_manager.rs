...
pub async fn start_machine_browser(
    ...
    // Inject the JIT proxy endpoint (stable across Chrome restarts) instead of the raw
    // CDP URL from Chrome. The raw URL uses --remote-debugging-port=0, so Chrome picks a
    // new random port on every launch. Injecting that URL means the gateway goes stale
    // the moment Chrome is killed and restarted, requiring `openclaw gateway restart` to
    // recover. The JIT proxy URL is deterministic (derived from agent_id hash) and
    // transparently forwards to whichever Chrome instance is currently live, so the
    // gateway can reconnect after a Chrome restart without any manual intervention.
    //
    // sync_agent_skills and boot_sync_agents already use this approach correctly; this
    // call makes start_machine_browser consistent with them.
    if let Ok(port) = enable_jit_proxy(app_handle, agent_id.clone()).await {
