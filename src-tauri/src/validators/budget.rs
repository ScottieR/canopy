...
pub fn validate_amount(cents: i64) -> Result<()> {
    if cents <= 0 {
        return Err(ValidationError::OutOfRange(
            format!("Amount must be positive, got: {} cents", cents)
        ).into());
    }
    ...
    if cents > 1_000_000_000 {
        return Err(ValidationError::OutOfRange(
            format!("Amount must be $10,000,000 or less, got: ${:.2}", cents as f64 / 100.0)
        ).into());
    }
...
pub fn validate_daily_limit(daily_cents: i64, monthly_cents: Option<i64>) -> Result<()> {
    if daily_cents <= 0 {
        return Err(ValidationError::OutOfRange(
            format!("Daily limit must be positive, got: {} cents", daily_cents)
        ).into());
    }
...
pub fn validate_monthly_limit(monthly_cents: i64, daily_cents: Option<i64>) -> Result<()> {
    if monthly_cents <= 0 {
        return Err(ValidationError::OutOfRange(
            format!("Monthly limit must be positive, got: {} cents", monthly_cents)
        ).into());
    }
...
pub fn validate_category(category: &str) -> Result<()> {
    ...
    if category.len() > 50 {
        return Err(ValidationError::TooLong(
            "Category must be 50 chars or less".into()
        ).into());
    }

    // Whitelist: alphanumeric, dash, underscore, space
    if !category.chars().all(|c| {
