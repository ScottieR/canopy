...
pub fn validate_openai_key(key: &str) -> Result<()> {
    ...
    if key.len() < 40 {
        return Err(ValidationError::InvalidFormat(format!(
            "OpenAI key is too short (minimum 40 chars, got: {} chars)",
            key.len()
        ))
        .into());
    }
...
pub fn validate_anthropic_key(key: &str) -> Result<()> {
    if key.is_empty() {
        return Err(ValidationError::InvalidFormat("Anthropic key cannot be empty".into()).into());
    }

    if key.len() < 32 {
        return Err(ValidationError::InvalidFormat(format!(
            "Anthropic key is too short (minimum 32 chars, got: {} chars)",
            key.len()
        ))
        .into());
    }
...
pub fn validate_gemini_key(key: &str) -> Result<()> {
    if key.is_empty() {
        return Err(ValidationError::InvalidFormat("Gemini key cannot be empty".into()).into());
    }

    if key.len() < 30 {
        return Err(ValidationError::InvalidFormat(format!(
            "Gemini key is too short (minimum 30 chars, got: {} chars)",
            key.len()
        ))
        .into());
    }
...
pub fn validate_xai_key(key: &str) -> Result<()> {
    if key.is_empty() {
        return Err(ValidationError::InvalidFormat("xAI key cannot be empty".into()).into());
    }

    if key.len() < 20 {
        return Err(ValidationError::InvalidFormat(format!(
            "xAI key is too short (minimum 20 chars, got: {} chars)",
            key.len()
        ))
        .into());
    }
...
pub fn validate_privacy_key(key: &str) -> Result<()> {
    if key.is_empty() {
        return Err(
            ValidationError::InvalidFormat("Privacy.com key cannot be empty".into()).into(),
        );
    }

    if key.len() < 20 {
        return Err(ValidationError::InvalidFormat(format!(
            "Privacy.com key is too short (minimum 20 chars, got: {} chars)",
            key.len()
        ))
        .into());
    }
...
pub fn validate_slack_secret(secret: &str) -> Result<()> {
    if secret.is_empty() {
        return Err(ValidationError::InvalidFormat("Slack secret cannot be empty".into()).into());
    }

    if secret.len() < 20 {
        return Err(ValidationError::InvalidFormat(format!(
            "Slack secret is too short (minimum 20 chars, got: {} chars)",
            secret.len()
        ))
        .into());
    }
...
pub fn validate_google_secret(secret: &str) -> Result<()> {
    if secret.is_empty() {
        return Err(ValidationError::InvalidFormat("Google secret cannot be empty".into()).into());
    }

    if secret.len() < 10 {
        return Err(ValidationError::InvalidFormat(format!(
            "Google secret is too short (minimum 10 chars, got: {} chars)",
            secret.len()
        ))
        .into());
    }
