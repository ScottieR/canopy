...
#[tauri::command]
pub async fn whitelist_bluetooth_device(
    agent_id: String,
    device_id: String,
    device_name: String,
) -> Result<(), String> {
    tracing::info!(
        "Whitelisting Bluetooth device {} for agent {}",
        device_id,
        agent_id
    );
...
#[tauri::command]
pub async fn get_whitelisted_bluetooth_devices(
    agent_id: String,
...
#[tauri::command]
pub async fn read_bluetooth_device_data(
    agent_id: String,
    device_id: String,
) -> Result<serde_json::Value, String> {
    tracing::info!(
        "Agent {} requesting to read Bluetooth device {}",
        agent_id,
        device_id
    );
    ...
    if !allowed_devices.iter().any(|d| d.id == device_id) {
        tracing::warn!(
            "SECURITY BLOCKED: Agent {} attempted to read non-whitelisted Bluetooth device {}",
            agent_id,
            device_id
        );
        return Err(format!(
            "Access Denied: You have not authorized agent {} to access device {}",
            agent_id, device_id
        ));
    }
    ...
    if !is_connected {
        ...
        let connect_result =
            tokio::time::timeout(Duration::from_secs(5), peripheral.connect()).await;
    ...
    for characteristic in peripheral.characteristics() {
            ...
            .contains(btleplug::api::CharPropFlags::READ)
        {
            // Read with timeout
            if let Ok(Ok(val)) =
                tokio::time::timeout(Duration::from_secs(2), peripheral.read(&characteristic)).await
            {
                // Return hex string of value
                let hex_val = hex::encode(&val);
                data.insert(
                    characteristic.uuid.to_string(),
                    serde_json::Value::String(hex_val),
                );
            }
        }
    }
