...
export function ForumView() {
  ...
  // Orphan detection only — the global orchestrator (App.tsx) handles engine lifecycle.
  // When ForumDetail mounts for the first time this session and finds an active forum
  // that already has agent messages, it means the app restarted mid-run.
  // We auto-resume via the store so the global orchestrator picks it back up.
  const orphanCheckedRef = useRef(false);

  useEffect(() => {
    if (!forum || orphanCheckedRef.current) return;
    orphanCheckedRef.current = true;
    if (forum.status !== "active") return;
    const hasAgentMessages = forum.messages.some(m => m.sender === "agent");
    if (!hasAgentMessages) return;
    // Forum was running when the app closed — auto-resume.
    // resumeForum increments orchestratorVersion which triggers the global
    // orchestrator to restart the engine.
    const t = setTimeout(() => useForumStore.getState().resumeForum(forum.id), 800);
