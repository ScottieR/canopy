...
// device.
//
// Previously this file was a "Dashboard" component that existed but was
// never wired into App.tsx's activeView router — dead code. It's now
// reachable via activeView === "dashboard".
...
interface AggregatedRole {
  role: string;
  emoji: string; // emoji of the first agent seen for this role
  agentCount: number;
  activeCount: number;
  tasksToday: number;
...
export function Dashboard() {
  ...
  // the backend), so fetch every agent's 90-day heatmap in parallel and sum
  // by date client-side, rather than adding a new Rust command for this.
    ...
    const fetchAll = async () => {
      ...
      try {
        const results = await Promise.all(
          agents.map(a =>
            invoke<HeatmapEntry[]>("get_agent_activity_heatmap", { agentId: a.id }).catch(() => [] as HeatmapEntry[])
          )
        );
        if (cancelled) return;
        const byDate = new Map<string, HeatmapEntry>();
        for (const perAgent of results) {
        ...
        setHeatmap(Array.from(byDate.values()).sort((a, b) => a.date.localeCompare(b.date)));
      } finally {
  ...
  // omitted — no backend changes needed here.
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        const data = await invoke<AuditEntry[]>("get_global_audit_log", { limit: 100 });
  ...
  const totals = useMemo(() => {
    let tasksToday = 0, messagesHandled = 0, tokensIn = 0, tokensOut = 0, costUsd = 0, spendLimit = 0, weeklyCompute = 0, uptimeSeconds = 0;
    for (const a of agents) {
      ...
      uptimeSeconds += a.stats?.uptime_seconds || 0;
      costUsd += a.stats?.total_cost_usd || (a as any).monthlySpend || 0;
      spendLimit += (a as any).spendLimit || 0;
      weeklyCompute += parseFloat((a as any).weeklyCompute) || 0;
    }
    return { tasksToday, messagesHandled, tokensIn, tokensOut, costUsd, spendLimit, weeklyCompute, uptimeSeconds };
  }, [agents]);

  // ── Per-role breakdown (existing behavior, now with cost added) ─────────
  const aggregatedRoles = useMemo(() => {
    ...
    for (const agent of agents) {
      ...
      aggr.agentCount++;
      if (agent.status === "active" || agent.status === "thinking") aggr.activeCount++;
      ...
      aggr.costUsd += agent.stats?.total_cost_usd || (agent as any).monthlySpend || 0;
    }
    ...
    <div style={{ padding: 24, maxWidth: 1200, margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ ...glass(0.5), padding: 16, borderRadius: 14 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
            <div>
              ...
              <div style={{ fontSize: 18, fontWeight: 600, color: "var(--text-main)" }}>{totals.weeklyCompute.toFixed(3)}</div>
            </div>
            <div>
          ...
          <ProgressBar value={totals.weeklyCompute} max={0.1 * Math.max(agents.length, 1)} color="#4A9E96" />
        </div>

        <div style={{ ...glass(0.5), padding: 16, borderRadius: 14 }}>
          <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.06em", color: "var(--text-sub)", textTransform: "uppercase", marginBottom: 8 }}>Cost (Total)</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        {aggregatedRoles.map((aggr) => (
          <div key={aggr.role} style={{ ...glass(0.5), padding: 16, borderRadius: 14 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
              <span style={{ fontSize: 24 }}>{aggr.emoji}</span>
              <div>
                <div style={{ fontWeight: 600, color: "var(--text-main)" }}>{aggr.role}</div>
                <div style={{ fontSize: 12, color: "var(--text-sub)" }}>{aggr.agentCount} Agent{aggr.agentCount !== 1 ? 's' : ''}</div>
              </div>
              <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ fontSize: 12, color: "var(--text-sub)" }}>{aggr.activeCount > 0 ? "Active" : "Idle"}</div>
                <div style={{ width: 10, height: 10, borderRadius: "50%", background: aggr.activeCount > 0 ? "#34D399" : "var(--text-muted)" }} />
              </div>
            </div>
            <div style={{ fontSize: 12, color: "var(--text-sub)", display: "flex", justifyContent: "space-between", marginTop: 12, paddingTop: 12, borderTop: "1px solid var(--border-subtle)" }}>
              <div><strong>{aggr.tasksToday}</strong> tasks · <strong>{aggr.messagesHandled}</strong> msgs</div>
              <div><strong>${aggr.costUsd.toFixed(2)}</strong> · <strong>{formatTokens(aggr.tokensIn + aggr.tokensOut)}</strong> tokens</div>
            </div>
          </div>
        ))}
      ...
      {/* ── Token Spend Chart — TokenSpendChart already aggregates across all agents when agentId is omitted ── */}
      ...
      <div style={{ ...glass(0.5), padding: "20px 24px", borderRadius: 16, display: "flex", flexDirection: "column", minHeight: 320 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.06em", color: "var(--text-sub)", textTransform: "uppercase" }}>Work Log — All Agents</div>
                ...
                return (
                  <div key={group.id} style={{ fontSize: 13, color: "var(--text-main)", padding: "10px 14px", background: "var(--surface-base)", borderRadius: 8, border: "1px solid var(--border-subtle)", display: "flex", flexDirection: "column", gap: 4 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{agentName(log.agent_id)} · {new Date(log.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                  ...
                  <div key={group.id} style={{ fontSize: 13, color: "var(--text-main)", background: "var(--surface-base)", borderRadius: 8, border: "1px solid var(--border-subtle)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
                    <div onClick={() => toggleSession(group.id)} style={{ padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", background: isExpanded ? "rgba(0,0,0,0.02)" : "transparent" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{new Date(group.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
