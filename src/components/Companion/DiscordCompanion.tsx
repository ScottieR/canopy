...
export function DiscordCompanion() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      open("https://discord.com/developers/applications").catch(console.error);
    }, 500);
  }, []);
    ...
    <div data-tauri-drag-region style={{
      ...
      <div style={{ padding: "0 24px 32px 24px", flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ 
          ...
          <h2 style={{ margin: 0, fontSize: 20, color: "#303330", fontWeight: 700, fontFamily: "'Noto Serif', Georgia, serif" }}>Connect Discord</h2>
          <p style={{ margin: "4px 0 0 0", fontSize: 13, color: "#636E72", textAlign: "center", padding: "0 16px" }}>
            Create a dedicated bot user for {agentName} to use on Discord.
          </p>
        </div>
        ...
        <div style={{ marginBottom: 24, padding: 16, background: "white", borderRadius: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.03)", border: "1px solid rgba(0,0,0,0.05)" }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#3c6663", marginBottom: 8 }}>Step 1: Create a Bot Application</div>
          <div style={{ fontSize: 13, color: "#4A5568", marginBottom: 16, lineHeight: 1.5 }}>
            <ol style={{ margin: "0 0 8px -5px", paddingLeft: "20px" }}>
              <li style={{ marginBottom: 8 }}>
                Open the <a href="#" onClick={(e) => { e.preventDefault(); open("https://discord.com/developers/applications"); }} style={{ color: "#3c6663", fontWeight: 600, textDecoration: "none" }}>Discord Developer Portal</a> in your browser.
              </li>
              <li style={{ marginBottom: 8 }}>Click <strong>New Application</strong> and give your bot a name.</li>
              <li>Go to the <strong>Bot</strong> tab on the left sidebar and scroll down to the <strong>Privileged Gateway Intents</strong> section. Enable <strong>Message Content Intent</strong>.</li>
            </ol>
          </div>
        </div>

        <div style={{ marginBottom: 24, padding: 16, background: "white", borderRadius: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.03)", border: "1px solid rgba(0,0,0,0.05)" }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#3c6663", marginBottom: 8 }}>Step 2: Copy Bot Token</div>
          <div style={{ fontSize: 13, color: "#4A5568", marginBottom: 12, lineHeight: 1.5 }}>
            Scroll back up on the Bot page, click <strong>Reset Token</strong>, and paste it below:
          </div>
          <PasswordInput 
            value={discordToken} 
        ...
        <button 
          ...
          disabled={!discordToken.trim() || testStatus === "testing" || testStatus === "success"}
          style={{
            marginTop: "auto",
            width: "100%",
            padding: "16px", borderRadius: 12, border: "none", 
            background: discordToken.trim() ? "#5865F2" : "rgba(0,0,0,0.06)", 
            color: discordToken.trim() ? "white" : "rgba(0,0,0,0.3)", 
            fontSize: 15, fontWeight: 700, 
            cursor: discordToken.trim() ? "pointer" : "default",
            transition: "all 0.2s"
          }}
        >
